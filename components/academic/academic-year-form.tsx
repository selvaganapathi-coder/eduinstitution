  ...
  async function submit(values: FormValues) {
    ...
    try {
      const response = await fetch("/api/academic/years", {
        ...
        body: JSON.stringify({ name: values.name, startDate: values.startDate, endDate: values.endDate }),
      });
    ...
    <Card className="!rounded-2xl !border-[#dadce0] !shadow-[0_1px_2px_rgba(60,64,67,.08)]">
      ...
      <Form layout="vertical" onFinish={submit} requiredMark="optional">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Form.Item name="startDate" label="Start date" rules={[{ required: true, message: "Choose a start date." }]}>
            <Input type="date" size="large" />
          </Form.Item>
          <Form.Item name="endDate" label="End date" rules={[{ required: true, message: "Choose an end date." }]}>
            <Input type="date" size="large" />
          </Form.Item>
        </div>
