...
export function TermManagement({ id }: { id: string }) {
  ...
  useEffect(() => { async function run() { try { await load(); } catch (reason: unknown) { setError(reason instanceof Error ? reason.message : "Unable to load terms"); } finally { setLoading(false); } } void run(); }, [id]);
  ...
  async function save(values: Values) {
    ...
    try {
      ...
      setEditing(null); form.resetFields(); await load();
  ...
  function openEdit(term: Term) { setEditing(term); form.setFieldsValue({ name: term.name, startDate: dateInput(term.startDate), endDate: dateInput(term.endDate), sortOrder: term.sortOrder }); }
  ...
  return <div className="space-y-5">
    ...
    <Modal open={editing !== null || form.getFieldValue("name") !== undefined} title={editing ? "Edit term" : "Add term"} open={editing !== null} onCancel={() => setEditing(null)} footer={null} destroyOnHidden>
      <Form form={form} layout="vertical" onFinish={save} className="pt-3" requiredMark="optional">
        ...
        <Form.Item name="sortOrder" label="Order" initialValue={1} rules={[{ required: true, message: "Enter the order." }]}><Input type="number" min={1} size="large" /></Form.Item>
        <div className="flex justify-end gap-3"><Button onClick={() => setEditing(null)}>Cancel</Button><Button type="primary" htmlType="submit" loading={saving}>Save term</Button></div>
      </Form>
    </Modal>
  </div>;
}
