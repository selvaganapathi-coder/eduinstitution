...
import { AttendanceSessionStatus } from "../../generated/prisma";
...
function createDb(overrides: Record<string, unknown> = {}) {
  ...
  const db = {
    attendanceSession: {
      findFirst: async () => ({ id: "session-1", tenantId: "tenant-1", academicGroupId: "group-1", attendanceDate: new Date("2026-08-29T00:00:00.000Z"), status: AttendanceSessionStatus.OPEN }),
    },
...
describe("markAttendance", () => {
  ...
  it("rejects a non-open attendance session", async () => {
    const { db } = createDb({ attendanceSession: { findFirst: async () => ({ id: "session-1", tenantId: "tenant-1", academicGroupId: "group-1", attendanceDate: new Date("2026-08-29"), status: AttendanceSessionStatus.FINALIZED }) } });
