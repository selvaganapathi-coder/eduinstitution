...
function failure(error: unknown) {
  ...
  return NextResponse.json({ error: "Unable to mark attendance." }, { status: 500 });
}
...
export async function PUT(request: Request) {
  try {
    ...
    if (!attendanceSessionId) return NextResponse.json({ error: "Attendance session is required." }, { status: 400 });

    const validation = validateAttendanceRecordInput(body);
    if ("error" in validation) return NextResponse.json(validation, { status: 400 });

    const record = await markAttendance(getPrisma(), {
