  ...
  return (
    <div
      ...
      className={`flex h-11 w-11 items-center justify-center rounded-xl ${
        inverse ? "bg-white/10 text-white" : "bg-[#e9f5ef] text-[#0c6b4f]"
      }`}
    >
...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f7fbf8] px-4 py-4 text-[#17352d] sm:px-6 lg:px-8 lg:py-8">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-[1240px] flex-col overflow-hidden rounded-[28px] bg-white shadow-[0_20px_60px_rgba(7,84,63,0.10)] lg:min-h-[calc(100vh-4rem)] lg:flex-row">
        <section className="relative flex min-h-[460px] w-full flex-col overflow-hidden bg-[#076653] p-7 text-white sm:p-10 lg:w-[43%] lg:p-12">
          ...
          <div className="relative z-10 mt-auto max-w-md pb-8 pt-20 lg:pt-28">
            <p className="mb-4 text-sm font-medium uppercase tracking-[0.16em] text-[#dff51f]">
              Built for education
            </p>
            <h2 className="text-4xl font-semibold leading-[1.08] tracking-[-0.04em] sm:text-5xl">
              Empowering education.
              <span className="block text-[#dff51f]">Together.</span>
            </h2>
            <p className="mt-6 max-w-sm text-base leading-7 text-white/75">
              A unified workspace for institutions, educators, students, and administrators.
            </p>
            ...
            <div className="mt-8 flex max-w-sm items-center gap-3 rounded-2xl border border-white/10 bg-white/10 px-4 py-3.5 backdrop-blur-sm">
              ...
              <div>
                ...
                <p className="text-xs leading-5 text-white/60">Tenant-scoped access and protected sessions.</p>
              </div>
            </div>
          </div>

          <div className="relative z-10 mt-auto hidden h-24 items-end gap-2 overflow-hidden lg:flex">
            {["h-10", "h-16", "h-12", "h-20", "h-14", "h-9"].map((height, index) => (
              <div key={index} className={`w-8 rounded-t-full bg-white/10 ${height}`} />
            ))}
          </div>
        </section>
        ...
        <section className="flex w-full items-center bg-white px-6 py-10 sm:px-10 lg:w-[57%] lg:px-16 xl:px-20">
          <div className="mx-auto w-full max-w-[520px]">
            <div className="mb-9 flex items-center gap-3 lg:hidden">
              <InstitutionMark />
              <div>
                <p className="font-semibold tracking-[-0.02em]" style={{ color: colors.ink }}>
                  EduInstitution
                </p>
            ...
            <div className="mb-8">
              <h1 className="text-[32px] font-semibold tracking-[-0.035em]" style={{ color: colors.ink }}>
                Sign in
              </h1>
              <p className="mt-2 text-[15px] leading-6" style={{ color: colors.muted }}>
                Welcome back. Sign in to continue to your institution workspace.
              </p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              <label className="block">
                <span className="mb-2 block text-sm font-medium" style={{ color: colors.ink }}>
                  Email
                </span>
                <input
                  type="email"
                  ...
                  className="h-12 w-full rounded-xl border bg-white px-4 text-[15px] outline-none transition placeholder:text-[#8a9993] focus:border-[#0c6b4f] focus:ring-4 focus:ring-[#0c6b4f]/10"
              ...
              <label className="block">
                <div className="mb-2 flex items-center justify-between gap-4">
                  ...
                  <span className="text-xs text-right" style={{ color: colors.muted }}>
                    Optional for single-tenant accounts
                  </span>
                </div>
                <input
                  value={tenantId}
              ...
              {error ? (
                <div role="alert" className="rounded-xl border border-[#f3c7c2] bg-[#fff6f5] px-4 py-3 text-sm leading-5 text-[#a5271d]">
                  {error}
                </div>
              ) : null}
              ...
              <button
                type="submit"
                disabled={loading}
                className="h-12 w-full rounded-xl px-6 text-sm font-semibold text-white shadow-[0_4px_12px_rgba(12,107,79,0.18)] transition hover:bg-[#07543f] focus:outline-none focus:ring-4 focus:ring-[#0c6b4f]/20 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <div className="flex items-start gap-3 rounded-xl bg-[#f5faf7] px-4 py-3.5">
              <div className="mt-0.5 text-[#0c6b4f]">
              ...
              <p className="text-xs leading-5" style={{ color: colors.muted }}>
                Your session is protected with tenant-scoped authentication and an HTTP-only security cookie.
              </p>
            </div>

            <p className="mt-8 text-center text-xs leading-5" style={{ color: colors.muted }}>
      ...
      <footer className="mx-auto flex w-full max-w-[1240px] flex-col items-center justify-between gap-3 px-2 py-5 text-xs sm:flex-row" style={{ color: colors.muted }}>
        <span>© 2026 EduInstitution. All rights reserved.</span>
