...
export default function InstitutionTypesPage() {
  ...
  useEffect(() => {
    ...
    async function fetchTypes() {
      ...
      try {
        ...
        if (active) setMessage({ kind: "error", text: error instanceof Error ? error.message : "We couldn't load institution types. Please try again." });
      } finally {
  ...
  async function saveCapabilities() {
    ...
    try {
      ...
      setDraft(Object.fromEntries((data.institutionType.capabilities as TypeCapability[]).map((item) => [item.capabilityId, item.enabled])));
  ...
  return (
    <ApplicationShell pageTitle="Institution types" pageContext="Choose which features are available to each type of institution" selectedKey="institution">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div><h1 className="mb-1 text-[28px] font-medium tracking-[-0.02em] text-[#202124]">Institution types</h1><p className="m-0 max-w-2xl text-sm leading-6 text-[#5f6368]">Manage the institution categories supported by EduInstitution and choose the features available to each category.</p></div>
        ...
        {!loading && types.length ? <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]"><aside className="space-y-2"><p className="mb-3 text-xs font-medium uppercase tracking-wide text-[#5f6368]">Institution types</p>{types.map((item) => <button key={item.id} type="button" onClick={() => selectType(item)} className={`w-full rounded-xl border px-4 py-3 text-left transition ${item.id === selectedId ? "border-[#a8c7fa] bg-[#e8f0fe]" : "border-[#dadce0] bg-white hover:border-[#bdc1c6]"}`}><div className="font-medium text-[#202124]">{item.name}</div><div className="mt-1 text-xs text-[#5f6368]">{item._count?.tenants ?? 0} institution{item._count?.tenants === 1 ? "" : "s"}</div></button>)}</aside>
          {selected ? <section className="rounded-2xl border border-[#dadce0] bg-white p-5 shadow-[0_1px_2px_rgba(60,64,67,.06)] sm:p-6"><div className="flex flex-col gap-4 border-b border-[#e8eaed] pb-5 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="m-0 text-xl font-medium text-[#202124]">{selected.name}</h2><p className="mt-1 mb-0 text-sm leading-6 text-[#5f6368]">{selected.description ?? "Configure the features available to this institution type."}</p></div><span className="inline-flex w-fit rounded-full bg-[#e6f4ea] px-3 py-1 text-xs font-medium text-[#137333]">{selected.status === "ACTIVE" ? "Active" : "Archived"}</span></div><div className="py-5"><h3 className="mb-1 text-base font-medium text-[#202124]">Available features</h3><p className="mb-4 text-sm text-[#5f6368]">Turn features on or off for this institution type. This does not delete existing data.</p><div className="divide-y divide-[#e8eaed] rounded-xl border border-[#dadce0]">{selected.capabilities.map((item) => <label key={item.capabilityId} className="flex cursor-pointer items-start gap-3 p-4 hover:bg-[#f8f9fa]"><input type="checkbox" checked={draft[item.capabilityId] ?? false} onChange={(event) => setDraft((current) => ({ ...current, [item.capabilityId]: event.target.checked }))} className="mt-1 h-4 w-4 accent-[#188038]" /><span className="min-w-0"><span className="block text-sm font-medium text-[#202124]">{item.capability.name}</span><span className="mt-1 block text-xs leading-5 text-[#5f6368]">{item.capability.description ?? "Available as part of this institution type."}</span></span></label>)}</div></div><div className="flex flex-col-reverse gap-3 border-t border-[#e8eaed] pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="m-0 text-xs leading-5 text-[#5f6368]"><CheckOutlined className="mr-1 text-[#188038]" />Changes apply to this institution type.</p><button type="button" onClick={() => void saveCapabilities()} disabled={saving} className="inline-flex h-10 items-center justify-center rounded-lg bg-[#188038] px-5 text-sm font-medium text-white transition hover:bg-[#137333] disabled:cursor-not-allowed disabled:opacity-60">{saving ? "Saving..." : "Save changes"}</button></div></section> : null}</div> : null}
      </div>
    </ApplicationShell>
  );
}
