...
export default function StudentsPage() {
  ...
  useEffect(() => {
    ...
    async function loadOptions() {
      try {
        const [yearResponse, departmentResponse, programResponse] = await Promise.all([
        ...
        const [yearData, departmentData, programData] = await Promise.all([
          yearResponse.json(),
        ...
        if (!cancelled) {
          notify("error", error instanceof Error ? error.message : "Unable to load academic options.");
        }
      }
    }
  ...
  function openEdit(student: Student) {
    ...
    setForm({
      ...
      departmentId: enrollment?.department.id ?? "",
    ...
    <ApplicationShell pageTitle="Students" pageContext="Student management" selectedKey="students">
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="m-0 text-[28px] font-normal tracking-[-0.02em] text-[#202124]">Students</h1>
            <p className="mb-0 mt-1 max-w-2xl text-sm leading-6 text-[#4b5563]">Manage student records, profile photos, and academic enrollment in one place.</p>
          </div>
        ...
        <section className="rounded-2xl border border-[#dadce0] bg-white p-4 shadow-[0_1px_2px_rgba(60,64,67,.06)]">
        ...
        <section className="overflow-hidden rounded-2xl border border-[#dadce0] bg-white shadow-[0_1px_2px_rgba(60,64,67,.06)]">
          {loading ? (
            <div className="flex min-h-[360px] items-center justify-center"><Spin size="large" /></div>
          ) : students.length === 0 ? (
            <div className="flex min-h-[360px] flex-col items-center justify-center p-8 text-center">
              <Avatar size={52} icon={<UserOutlined />} className="mb-4 !bg-[#e6f4ea] !text-[#137333]" />
              <h2 className="m-0 text-lg font-medium text-[#202124]">No students found</h2>
              <p className="mt-2 max-w-md text-sm leading-6 text-[#4b5563]">Add your first student or change the current search and filters.</p>
              <Button type="primary" icon={<PlusOutlined />} onClick={openCreate} className="mt-4 !bg-[#188038]">Add student</Button>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="min-w-[980px] w-full border-collapse">
                  <thead className="bg-[#f8f9fa]">
                    <tr className="text-left text-xs font-medium uppercase tracking-wide text-[#374151]">
                      <th className="px-5 py-3">Student</th><th className="px-5 py-3">Program</th><th className="px-5 py-3">Contact</th><th className="px-5 py-3">Academic year</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {students.map((student) => {
                      ...
                      return (
                        <tr key={student.id} className="border-b border-[#eef0f1] last:border-b-0 hover:bg-[#f8faf8]">
      ...
      <Drawer title={editing ? "Edit student" : "Add student"} placement="right" width={520} open={drawerOpen} onClose={() => setDrawerOpen(false)} extra={<Button type="primary" loading={saving} onClick={() => void saveStudent()} className="!bg-[#188038]">Save</Button>}>
        <div className="space-y-5">
          <div className="rounded-xl border border-[#eef0f1] p-4"><div className="mb-3 flex items-center gap-3"><Avatar size={52} src={form.photoUrl || undefined} icon={!form.photoUrl ? <UserOutlined /> : undefined} /><div><div className="font-medium text-[#202124]">Student photo</div><div className="text-sm text-[#4b5563]">Paste a hosted image URL for now.</div></div></div><Input value={form.photoUrl} onChange={(e) => setForm({ ...form, photoUrl: e.target.value })} placeholder="https://example.com/student-photo.jpg" /></div>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-medium text-[#374151]">Student number<Input className="mt-1.5" value={form.studentNumber} onChange={(e) => setForm({ ...form, studentNumber: e.target.value })} placeholder="Example: STU-2026-001" /></label>
            <label className="text-sm font-medium text-[#374151]">Enrollment number<Input className="mt-1.5" value={form.enrollmentNumber} onChange={(e) => setForm({ ...form, enrollmentNumber: e.target.value })} placeholder="Example: ENR-2026-001" /></label>
            <label className="text-sm font-medium text-[#374151]">First name<Input className="mt-1.5" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} /></label>
            <label className="text-sm font-medium text-[#374151]">Last name<Input className="mt-1.5" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} /></label>
            <label className="text-sm font-medium text-[#374151]">Email<Input className="mt-1.5" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label>
            <label className="text-sm font-medium text-[#374151]">Phone<Input className="mt-1.5" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label>
          </div>
          <div className="border-t border-[#eef0f1] pt-5">
            <h3 className="m-0 text-base font-medium text-[#202124]">Academic enrollment</h3>
            <p className="mt-1 text-sm text-[#4b5563]">Select the active academic context for this student.</p>
